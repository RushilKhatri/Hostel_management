...
import com.Rushil.HostelAllocation.Repository.HostelRepository;
import com.Rushil.HostelAllocation.Repository.StudentRepository;
import com.Rushil.HostelAllocation.Utility.HostelUtility;
            ...
            if (Objects.equals(studentId, "null")) {
                ...
                if (existHostel.get().getId() == hostelId && updatedStudentOptional.isPresent()) {
                    ...
                    if (existingHostelOptional.isPresent()) {
                        ...
                        // CHECK: If this room is already allocated to a different student
                        // Remove that student's allocation first (reallocation logic)
                        if (existingHostel.getStudent() != null &&
                                existingHostel.getStudent().getSt_id() != updatedStudent.getSt_id()) {
                            // Room is occupied by a different student - this is a reallocation
                            // The previous student will be automatically removed when we set the new
                            // student
                            System.out.println("Reallocating room " + existingHostel.getRoom() +
                                    " from student ID " + existingHostel.getStudent().getSt_id() +
                                    " to student ID " + updatedStudent.getSt_id());
                        }

                        // Assign the new student to the room (overwrites previous student if any)
